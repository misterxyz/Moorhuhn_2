...
class Huhn(Turtle):
    ...
    def schritt_links_rechts(self):
        Strafpunkt_links_rechts                          #die Strafpunkte beziehen sich auf nicht abgeschossene und damit entkommene Hühner
        Strafpunkt_links_rechts=0   
        if self.ausdemspiel_links:
            ...
            return
                if self.tot:
                    ...
                    y = y + self.vx                                         #hier die Y-Koordinate des herunterfallenden Huhns. Bei 'self.vy**2' fallen die Hühner nach oben :P
                    self.goto(x,y)
...
class MoorhuhnSpiel(object):
    ...
    def klang(self, sountfile):
        WINDSOUND.PlaySound(soundfile  winsound.SOND_ASYNC)   #der Befehl 'SND_ASYNC' sorgt dafür, daß das Abspielen des Klangs das Spiel nicht unterbricht, bzw. anhält 
