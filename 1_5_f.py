...
class Bildmanager(Turtle):   					#Konfiguration des Bildes
    def __init__(self, breite, höhe): 				#Spezielle Turtle, hat die Aufgabe die Moorhuhn-GUI zu kontrollieren
        Turtle__init__(self, visible=False)
        self.screen = Screen()
        ...
        selv.pencolor("yellow")                                 #Schriftfarbe
...
class Huhn(Turtle):
    def __init__(self, bilddatei, moorhuhnspiel):
        ...
        self.penup()
        self.speed(0)
        self.onclik(self.getrofen                            #wenn auf das Huhn draufgeklickt wurde, gilt es als 'getroffen'
        self.start_links()
    ...
    def start_links(self):                
        ...
        self.setpos(-BREITE//2-20,                              #das ist der Bereich, von wo aus die Hühner starten; '-20' bewirkt, daß sie etwas außerhalb vom Bild starten
                    random.randint(-HÖHE//3,
                                   HÖHE//3))    
        self.vx = random.randint(6,11) * GESCHWNDIGKEIT        #'vx' ist die Geschwindigkeit in X-Richtung
...
class MoorhuhnSpiel(object):
    def __init__(self):						#Kombiniert Elemente des Moorhuhn Spiels.
        ...
        bild.screen.register__shape"huhn02.gif")                #....Bilddatei als Turtle shape registriert. Danach kann die Methode 'shape()' aufgerufen werden, um der Turtle diese Gestalt....

        self.huehner_links = [Huhn("huhn01.gif", self),
                              Huhn("huhn02.gif", self)]
        self.gameover = False                                    #jetzt kann ein neues Spiel beginnen
